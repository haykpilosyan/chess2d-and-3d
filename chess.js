...
var chess = (function() {
    ...
    function showSolidBoard() {
        if (graphicsStatus === 0) {
            ...
            XHR("board.json", function() {
                if (graphicsStatus === 0) { return; }
                etc.tmp3DBoard = eval("(" + this.responseText + ")");
                runComponents();
            });
            XHR("pawn.jscn", loadCom, 0);
            XHR("king.jscn", loadCom, 1);
            XHR("knight.jscn", loadCom, 2);
            XHR("bishop.jscn", loadCom, 3);
            XHR("rook.jscn", loadCom, 4);
            XHR("queen.jscn", loadCom, 5);
            XHR("scene.jsfb", loadCom, 6);
            XHR("vec3.jsfb", loadCom, 7);
            XHR("matrix3.jsfb", loadCom, 8);
            XHR("camera.jsfb", loadCom, 9);
            XHR("mesh.jsfb", loadCom, 10);
            XHR("light.jsfb", loadCom, 11);
